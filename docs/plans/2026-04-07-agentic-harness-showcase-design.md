...
/catalog/skills                ← 내부 하네스 인벤토리 자동 생성 (research 05)
/catalog/agents                ← 동일
/catalog/hooks                 ← 동일
/catalog/rules                 ← 동일
...
| **C. 카탈로그 시스템** | `wt-catalog` | `data/catalog/*.json` (내부 하네스 인벤토리에서 빌드), `components/catalog/*`, `app/catalog/[type]/page.tsx`, `scripts/build-catalog.ts` | A 완료 후 (research 05도 필요) |
...
| `05-source-inventory.md` | `data/catalog/*.json` 자동 생성 입력 | C |
