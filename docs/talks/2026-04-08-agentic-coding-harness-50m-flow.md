...
주제: **Agentic Coding Harness 엔지니어링 — 모델보다 런타임이 중요해지는 이유**
...
| 0:04~0:12 | **1. 큰그림** | `/handbook` (#past · #present) | 2017 → 2026 흐름. 모델 → 런타임으로 무게 중심 이동 |
...
> "오늘 발표는 AI 코딩 도구 비교가 아닙니다. Claude Code 가 좋은지 Cursor 가 좋은지 같은 이야기는 일절 하지 않습니다. 대신 **하네스 엔지니어링 (harness engineering)** 이라는, 같은 모델을 쓰면서도 팀 생산성을 5~10배 차이나게 만드는 운영 층 이야기를 50분 동안 하겠습니다."
...
> "2017년에 Karpathy 가 'Software 2.0' 을 발표했을 때만 해도, 사람들은 LLM 을 '큰 자동완성' 정도로 봤습니다. 2023년의 LLM as OS 관점, 2024년의 'Building Effective Agents' 가 나오면서 'workflow vs agent' 구분이 생겼고, 2025~2026년에는 Ralph loop, eval-driven, context engineering, MCP 가 한 묶음이 되면서 지금의 **하네스 엔지니어링** 이라는 실전 감각이 생겼습니다."

> "여기서 가장 중요한 변화 한 가지만 기억하시면 됩니다. **체감 생산성은 모델 자체보다 런타임에서 결정됩니다**. 같은 Claude Opus 4.6 을 두 팀이 써도 어떤 팀은 5배 빠르고 어떤 팀은 그대로입니다. 차이는 그 모델을 둘러싼 작업 환경 — 즉 하네스 — 에 있습니다."

**짚을 것**:
- `<HandbookTimeline />` 5개 카드 (2017 → 2026 흐름)
- `<HarnessLandscape />` 3-레이어 stack (Foundation Models · Harness Runtime · External Extensions)
- "지금의 핵심 변화" callout
...
> "이론은 여기까지 하고 진짜 셋업으로 넘어갑니다. 두 런타임 — Claude Code 와 Codex — 을 같은 사상으로 어떻게 셋업하는지 나란히 보여드리겠습니다. 같은 일을 다른 파일 이름으로 합니다."
...
- `/handbook#claude-codex` 의 "같은 개념의 자리" 표를 한 번 띄워서 두 런타임 매핑 확인

> "여기서 핵심은, 두 런타임이 사상은 같고 문법만 다르다는 것입니다. 같은 메모리 내용을 CLAUDE.md / AGENTS.md 양쪽에 복사하면 어느 도구를 쓰든 같은 팀 표준이 됩니다."
...
> "이 기능 자체보다 더 중요한 건 **방향** 입니다. '어디에서 계산을 돌릴 것인가' 가 새 변수가 됐다는 거예요. 이 변수는 앞으로 다른 런타임에서도 비슷한 형태로 다시 나타날 가능성이 높습니다."
