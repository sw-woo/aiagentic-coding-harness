  ...
  return (
    <div className="border-b border-border bg-background py-16 sm:py-24">
      <Prose size="wide">
        ...
        <h1 className="balanced-korean-heading max-w-[14ch] font-serif text-4xl leading-[1.15] tracking-tight text-foreground sm:text-5xl">
          {meta.title}
        </h1>
        <p className="pretty-korean-copy mt-4 max-w-[62ch] font-serif text-lg leading-relaxed text-foreground-muted">
          {meta.description}
        </p>
...
function Karpathy() {
  return (
    <>
      <ProseParagraph>
        LLM 으로 코드를 만든다는 것은 결국 무엇을 뜻할까요. 단순히 자동완성이 조금 더 똑똑해진 것인지,
        아니면 정말 새로운 프로그래밍 패러다임이 열린 것인지부터 헷갈리기 쉽습니다.
      </ProseParagraph>
      <ProseParagraph>
        Andrej Karpathy 는 지난 몇 년 동안 이 질문에 가장 분명한 framing 을 제공한 인물 중 하나입니다.
        Software 1.0 · 2.0 · 3.0, 그리고 LLM as Operating System 이라는 관점은 지금 우리가 만드는 agentic coding harness 를
        이해하는 데 여전히 유용한 출발점입니다. 이 글에서는 그 framing 을 사실 기반으로 짧고 명확하게 정리합니다.
      </ProseParagraph>
      ...
      <ProseParagraph>
        쉽게 풀면 이렇습니다. Software 1.0 에서는 “고양이 사진을 알아내는 함수를 적어 보세요” 라고 하면 사람이 직접 규칙을 적어야 했습니다.
        Software 2.0 에서는 데이터셋과 목표만 주고, 학습이 그 목표를 만족하는 가중치를 찾아 줍니다. 사람은 가중치를 읽지 못해도 함수는 동작합니다.
      </ProseParagraph>
...
permissions ≈ syscall ACL — 어떤 명령을 누가 호출할 수 있는지의 정책`}
      ...
      <ProseParagraph>
        ...
        production 의 사고로 이어지면 다른 이름이 필요합니다. 구조화된 에이전틱 코딩 하네스(이 사이트가 보여드리는 그것) 는 vibe coding 의
        즉흥성과 검증 가능한 엔지니어링 사이의 차이를 명시적으로 다룹니다 — hooks, rules, eval loops, verification gates 가 그 답입니다.
      </ProseParagraph>
      <Callout tone="warning" title="vibe coding 과 구조화된 하네스의 차이">
